...
int main (int argc, char **argv)
{
  ...
  int shift_blockDimX = atoi(argv[3]);
  int shift_blockDimY= atoi(argv[4]);
  int shift_gridDimX= atoi(argv[1]);
  int shift_gridDimY= atoi(argv[2]);

  //printf("gridDimX -> ");
  //scanf("%d", &shift_gridDimX);
  
  //printf("gridDimY -> ");
  //scanf("%d", &shift_gridDimY);

  //printf("blockDimX -> ");
  //scanf("%d", &shift_blockDimX);

  //printf("blockDimY -> ");
  //scanf("%d", &shift_blockDimY); 

  printf("gridDimX = %d ",shift_gridDimX);
  printf("gridDimY = %d ",shift_gridDimY);
  printf("blockDimX = %d ",shift_blockDimX);
  printf("blockDimX = %d\n",shift_blockDimY);
  ...
  */
  ...
  blockDimX = 1 << shift_blockDimX;                                             // optimize!
  blockDimY = 1 << shift_blockDimY;                                             // optimize!
  blockDimZ = 1 << 0;                                             // do not change!
  gridDimX = 1 << shift_gridDimX;                                             // optimize!
  gridDimY = 1 << shift_gridDimY;                                              // optimize!
